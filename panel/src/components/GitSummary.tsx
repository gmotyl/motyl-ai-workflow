...
export default function GitSummary() {
  ...
  if (files.length === 0) {
    return <p className="text-xs px-1" style={{ color: "var(--text-muted)" }}>Clean</p>;
  }
  ...
  return (
    ...
    >
      {modified > 0 && <span style={{ color: "var(--yellow)" }}>{modified}M</span>}
      {newFiles > 0 && <span style={{ color: "var(--green)" }}>{newFiles}U</span>}
