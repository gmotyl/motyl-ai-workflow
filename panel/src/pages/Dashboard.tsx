...
import { FolderOpen, FileText, TrendingUp, Map as MapIcon, Database, Star } from "lucide-react";
...
function Badge({ label, icon: Icon }: { label: string; icon: typeof FileText }) {
  return (
    <span className="inline-flex items-center gap-1 px-1.5 py-0.5 text-[11px] rounded" style={{ background: "var(--bg-hover)", color: "var(--text-tertiary)" }}>
      <Icon size={10} />
      {label}
    </span>
  );
...
function ProjectCard({ project, isFav, onToggleFav }: { project: Project; isFav: boolean; onToggleFav: () => void }) {
  ...
  return (
    <div
      ...
      style={{ background: "var(--bg-surface)", border: `1px solid ${isFav ? "var(--accent-dim)" : "var(--border-subtle)"}` }}
      ...
      onMouseLeave={(e) => { e.currentTarget.style.borderColor = isFav ? "rgba(229,168,75,0.15)" : "var(--border-subtle)"; e.currentTarget.style.background = "var(--bg-surface)"; }}
    >
      <button
        ...
        <div className="mb-2">
          <ul className="space-y-0.5">
            {project.currentPlans.slice(0, 2).map((plan) => {
              ...
              return (
                <li key={plan} className="text-[11px] font-mono truncate" style={{ color: "var(--accent)", opacity: 0.7 }}>
                  {name}
                </li>
              );
            })}
